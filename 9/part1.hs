...
solve :: [[Int]] -> Int
solve [c] = last c       -- We have already solved all lines, so we can just return the last one
solve lines = solve (init (init lines) ++ [last (init lines) ++ [last (last (init lines)) + last (last lines)]])
